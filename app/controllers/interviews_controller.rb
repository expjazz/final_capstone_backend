...
class InterviewsController < ApplicationController
  def create
    if params[:company_id]
      ...
      @interview = Interview.create(candidate:
        @candidate, company: @company,
                                    job_offer_id:
                                    params[:job_offer_id], status: 'waiting for confirmation from the company',
                                    time: params[:time])
    end
  ...
  def update
    @interview = Interview
      .find_by(candidate_id: params[:candidate_id], company_id: params[:company_id], job_offer_id: params[:job_offer_id])
